...
import { ArrowLeft, Sparkles, Loader2, BookOpen, Lightbulb, Code2, Palette, Zap } from "lucide-react";
...
const VARIATION_STYLES = [
  ...
  },
  {
    id: "interactive",
    name: "Interactive",
    description: "Designed for interactive learning with questions and challenges",
    icon: Zap,
    color: "yellow",
  },
];

export default function GenerateScrimbaScriptPage({ params }: PageProps) {
    ...
    try {
      const response = await fetch("/api/ai/theory-variation", {
        ...
        body: JSON.stringify({
          ...
          style: selectedStyle,
          title: customTitle || VARIATION_STYLES.find(s => s.id === selectedStyle)?.name || "Script Variation",
        }),
      });

      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || "Failed to generate scrimba script");
      }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      <div className="space-y-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            Generate Scrimba Script
          </h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Create AI-generated theory content (Scrimba Script) for this lesson with different teaching styles
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Select Script Style</CardTitle>
          <CardDescription>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder={VARIATION_STYLES.find(s => s.id === selectedStyle)?.name || "Script Variation"}
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">
              Give this script a custom name to help identify it later
            </p>
          </div>
